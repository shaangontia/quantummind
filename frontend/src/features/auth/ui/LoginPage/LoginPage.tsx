...
export const LoginPage = () => {
  ...
  const [error, setError] = useState<string | null>(
    errorParam === 'google_denied' ? 'Sign-in cancelled.' :
    errorParam === 'google_failed' ? 'Google sign-in failed. Please try again or use email.' :
  ...
  return (
    <Box display="flex" alignItems="center" justifyContent="center" minHeight="100vh" bgcolor="background.default">
      <Paper elevation={0} sx={{ p: 4, width: '100%', maxWidth: 400, mx: 2 }}>
        ...
        <Typography variant="h5" fontWeight={700} textAlign="center" mb={0.5}>Welcome back</Typography>
        <Typography variant="body2" color="text.secondary" textAlign="center" mb={3}>
          Sign in to your AI trading portfolio
        </Typography>
        ...
        {!showEmail ? (
          ...
          <Box component="form" onSubmit={e => void handleSubmit(e)} display="flex" flexDirection="column" gap={2}>
            <TextField
              label="Email" type="email" autoComplete="email" required fullWidth
              value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com"
            />
            <TextField
              label="Password" type="password" autoComplete="current-password" required fullWidth
        ...
        <Typography variant="body2" color="text.secondary" textAlign="center" mt={2.5}>
          No account?{' '}
          <Box component={Link} to="/register" sx={{ color: 'primary.main', '&:hover': { textDecoration: 'underline' } }}>
            Create one
          </Box>
        </Typography>
      </Paper>
