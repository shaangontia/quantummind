...
exports.bootstrapSignalWeights = bootstrapSignalWeights;
const turso_js_1 = require("../db/turso.js");
const backtestData_js_1 = require("./backtestData.js");
const backtestEngine_js_1 = require("./backtestEngine.js");
 ...
 */
function winRateToWeight(winRate, totalSignals) {
 ...
 */
async function upsertWeight(source, summary) {
    const weight = winRateToWeight(summary.winRate, summary.totalSignals);
    await (0, turso_js_1.run)(`INSERT OR REPLACE INTO signal_weights
    ...
    logger_js_1.logger.info({ reason: `[BacktestWeights] ${source}: winRate=${(summary.winRate * 100).toFixed(1)}% (${summary.totalSignals} signals) => weight=${weight.toFixed(3)}` });
}
/**
 ...
 */
async function bootstrapSignalWeights(symbolOverride) {
    const symbols = symbolOverride ?? marketData_js_1.NSE_UNIVERSE;
    logger_js_1.logger.info({ reason: `[BacktestWeights] Starting bootstrap for ${symbols.length} symbols` });
    // Step 1: Fetch historical data
    const { fetched, skipped, failed } = await (0, backtestData_js_1.fetchAndStoreHistory)(symbols, {
        skipExisting: true,
        delayMs: 300,
    });
    logger_js_1.logger.info({ reason: `[BacktestWeights] Data fetch: ${fetched} new, ${skipped} skipped, ${failed.length} failed` });
    // Step 2: Run backtest
    const { summaries, totalSignalsProcessed } = await (0, backtestEngine_js_1.runBacktest)(symbols);
    // Step 3: Write weights
    let weightsWritten = 0;
    for (const summary of summaries) {
        if (summary.totalSignals < 10) {
            logger_js_1.logger.warn({ reason: `[BacktestWeights] ${summary.signalType}: too few signals (${summary.totalSignals}), skipping` });
            continue;
        }
    ...
    const existingSources = (await (0, turso_js_1.query)('SELECT source FROM signal_weights')).map((r) => r.source);
    const allExpected = ['rsi', 'momentum', 'range', 'combined', 'news', 'regime'];
    for (const src of allExpected) {
        if (!existingSources.includes(src)) {
            await (0, turso_js_1.run)(`INSERT OR IGNORE INTO signal_weights (source, weight, win_rate, total_signals, winning_signals, last_updated)
         VALUES (?, 1.0, 0.5, 0, 0, CURRENT_TIMESTAMP)`, [src]);
            logger_js_1.logger.info({ reason: `[BacktestWeights] Initialised missing source: ${src} → 1.0` });
        }
    }
    logger_js_1.logger.info({ reason: `[BacktestWeights] Bootstrap complete — ${weightsWritten} weights written` });
    return {
