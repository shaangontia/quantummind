...
exports.isUnderDailyTradeLimit = isUnderDailyTradeLimit;
exports.isUnderDailyTurnoverLimit = isUnderDailyTurnoverLimit;
exports.isUnderPositionCap = isUnderPositionCap;
exports.acquireCycleLock = acquireCycleLock;
exports.acquireDbCycleLock = acquireDbCycleLock;
exports.releaseCycleLock = releaseCycleLock;
exports.ensureTradingConfigTable = ensureTradingConfigTable;
...
async function isTradingEnabled() {
    ...
    // Soft DB toggle
    try {
        const row = await (0, turso_js_1.queryOne)('SELECT value FROM trading_config WHERE key = ?', ['global_trading_enabled']);
...
async function isUnderDailyTradeLimit(portfolioId) {
    try {
        const row = await (0, turso_js_1.queryOne)(`SELECT COUNT(*) as count FROM trades
       WHERE portfolio_id = ?
         AND date(created_at) = date('now')`, [portfolioId]);
...
async function isUnderDailyTurnoverLimit(portfolioId, portfolioNAV, tradeAmount) {
    try {
        const row = await (0, turso_js_1.queryOne)(`SELECT COALESCE(SUM(amount), 0) as turnover FROM trades
       WHERE portfolio_id = ?
         AND date(created_at) = date('now')`, [portfolioId]);
...
async function isUnderPositionCap(portfolioId, symbol, portfolioNAV, additionalAmount) {
    try {
        const row = await (0, turso_js_1.queryOne)(`SELECT quantity, current_price FROM holdings WHERE portfolio_id = ? AND symbol = ?`, [portfolioId, symbol]);
...
async function acquireDbCycleLock() {
    try {
        await (0, turso_js_1.run)(`CREATE TABLE IF NOT EXISTS cron_lock (
      ...
      owner TEXT NOT NULL
        ...
        const existing = await (0, turso_js_1.queryOne)('SELECT locked_until FROM cron_lock WHERE key=?', [CRON_LOCK_KEY]);
        if (existing) {
            ...
            // Lock expired — update
            await (0, turso_js_1.run)('UPDATE cron_lock SET locked_until=?, owner=? WHERE key=?', [until, process.pid?.toString() ?? 'serverless', CRON_LOCK_KEY]);
        }
        else {
            await (0, turso_js_1.run)('INSERT INTO cron_lock (key, locked_until, owner) VALUES (?,?,?)', [CRON_LOCK_KEY, until, process.pid?.toString() ?? 'serverless']);
        }
        return true;
    }
    catch (err) {
...
async function releaseCycleLock() {
    try {
        await (0, turso_js_1.run)('DELETE FROM cron_lock WHERE key=?', [CRON_LOCK_KEY]);
    }
    catch { /* ignore */ }
    lastCycleRanAt = null; // also reset in-memory lock
}
async function ensureTradingConfigTable() {
    try {
        await (0, turso_js_1.run)(`CREATE TABLE IF NOT EXISTS trading_config (
      key   TEXT PRIMARY KEY,
      value TEXT NOT NULL,
      updated_at DATETIME DEFAULT (CURRENT_TIMESTAMP)
    )`);
        // Seed default if not present
        await (0, turso_js_1.run)(`INSERT OR IGNORE INTO trading_config (key, value) VALUES ('global_trading_enabled', 'true')`);
    }
    catch (err) {
