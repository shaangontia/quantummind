...
function detectSignals(rows) {
    ...
    for (let i = 60; i < rows.length - HOLD_DAYS; i++) {
        ...
        const toSignal = (type) => {
            if (exitPrice === null)
                return { symbol, date: current.date, signalType: type, entryPrice, exitPrice: null, holdDays: HOLD_DAYS, returnPct: null, outcome: 'open' };
        ...
        if (rsiOversold)
            signals.push(toSignal('rsi_oversold'));
        if (momentumBreak)
            signals.push(toSignal('momentum_breakout'));
        if (rangeLow)
            signals.push(toSignal('range_low'));
        if (rsiOversold && momentumBreak)
            signals.push(toSignal('combined'));
    }
    return signals;
}
// ─── Aggregate to summary per signal type ─────────────────────────────────────
function summarise(signals) {
    const types = ['rsi_oversold', 'momentum_breakout', 'range_low', 'combined'];
 ...
 */
async function runBacktest(symbols) {
    const allSignals = [];
    let symbolsProcessed = 0;
    for (const symbol of symbols) {
        try {
            const rows = await (0, backtestData_js_1.loadSymbolHistory)(symbol);
            if (rows.length < 60) {
                logger_js_1.logger.debug({ reason: `[Backtest] ${symbol}: not enough data (${rows.length} rows)` });
                continue;
            }
            const signals = detectSignals(rows);
            allSignals.push(...signals);
            symbolsProcessed++;
            logger_js_1.logger.debug({ reason: `[Backtest] ${symbol}: ${signals.length} signals generated` });
        }
        catch (err) {
            logger_js_1.logger.warn({ reason: `[Backtest] ${symbol}: error — ${err}` });
        }
    }
    const summaries = summarise(allSignals);
    const summaryStr = summaries.map(s => `${s.signalType}: ${(s.winRate * 100).toFixed(1)}% WR`).join(', ');
    logger_js_1.logger.info({ reason: `[Backtest] Complete — ${symbolsProcessed} symbols, ${allSignals.length} signals: ${summaryStr}` });
